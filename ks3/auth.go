...
func (conn Conn) signHeader(req *http.Request, canonicalizedResource string) {
	...
	if akIf.GetAccessKeyID() == "" && akIf.GetAccessKeySecret() == "" && akIf.GetSecurityToken() == "" {
		return
	}
	authorizationStr := ""
	if conn.config.AuthVersion == AuthV2 {
		...
		if len(additionalList) > 0 {
			authorizationFmt := "KS32 AccessKeyId:%v,AdditionalHeaders:%v,Signature:%v"
			...
			authorizationFmt := "KS32 AccessKeyId:%v,Signature:%v"
		...
		authorizationStr = "KSS " + akIf.GetAccessKeyID() + ":" + conn.getSignedStr(req, canonicalizedResource, akIf.GetAccessKeySecret())
	}
...
func (conn Conn) getSignedStr(req *http.Request, canonicalizedResource string, keySecret string) string {
	...
	// Sort the ks3HeadersMap by the ascending order
	hs.Sort()

	// Get the canonicalizedKS3Headers
	...
	// v2 signature
	if conn.config.AuthVersion == AuthV2 {
		signStr = req.Method + "\n" + contentMd5 + "\n" + contentType + "\n" + date + "\n" + canonicalizedKS3Headers + strings.Join(additionalList, ";") + "\n" + canonicalizedResource
