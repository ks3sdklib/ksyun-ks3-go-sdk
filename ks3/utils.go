...
func userAgent() string {
	sys := getSysInfo()
	return fmt.Sprintf("ksyun-sdk-go/%s (%s/%s/%s;%s)", Version, sys.name,
...
func SplitFileByPartNum(fileName string, chunkNum int) ([]FileChunk, error) {
	...
	if int64(chunkNum) > stat.Size() {
		return nil, errors.New("ks3: chunkNum invalid")
	}
...
func ChoiceTransferPartOption(options []Option) []Option {
	...
	payer, _ := FindOption(options, HTTPHeaderKs3Requester, nil)
	if payer != nil {
	...
	trafficLimit, _ := FindOption(options, HTTPHeaderKs3TrafficLimit, nil)
	if trafficLimit != nil {
...
func ChoiceCompletePartOption(options []Option) []Option {
	...
	acl, _ := FindOption(options, HTTPHeaderKs3ObjectACL, nil)
	if acl != nil {
		outOption = append(outOption, ObjectACL(ACLType(acl.(string))))
	}

	callback, _ := FindOption(options, HTTPHeaderKs3Callback, nil)
	if callback != nil {
		outOption = append(outOption, Callback(callback.(string)))
	}

	callbackVar, _ := FindOption(options, HTTPHeaderKs3CallbackVar, nil)
	if callbackVar != nil {
	...
	forbidOverWrite, _ := FindOption(options, HTTPHeaderKs3ForbidOverWrite, nil)
