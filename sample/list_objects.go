...
func ListObjectsSample() {
	...
	lor, err = bucket.ListObjects(ks3.MaxKeys(3))
	if err != nil {
		HandleError(err)
	}
	...
	lor, err = bucket.ListObjects(ks3.Prefix("my-object-2"))
	if err != nil {
		HandleError(err)
	}
	...
	lor, err = bucket.ListObjects(ks3.Marker("my-object-22"))
	if err != nil {
		HandleError(err)
	}
	...
	marker := ks3.Marker("")
	for {
		lor, err = bucket.ListObjects(ks3.MaxKeys(3), marker)
		if err != nil {
			HandleError(err)
		}
	...
	marker = ks3.Marker("my-object-22")
	for {
		lor, err = bucket.ListObjects(ks3.MaxKeys(3), marker)
		if err != nil {
			HandleError(err)
	...
	pre := ks3.Prefix("my-object-2")
	marker = ks3.Marker("")
	for {
		lor, err = bucket.ListObjects(ks3.MaxKeys(2), marker, pre)
		if err != nil {
			HandleError(err)
		}
		pre = ks3.Prefix(lor.Prefix)
		marker = ks3.Marker(lor.NextMarker)
	...
	lor, err = bucket.ListObjects(ks3.Prefix("fun/"), ks3.Delimiter("/"))
	if err != nil {
		HandleError(err)
	}
	fmt.Println("my objects prefix :", getObjectsFormResponse(lor),
...
func getObjectsFormResponse(lor ks3.ListObjectsResult) string {
	var output string
