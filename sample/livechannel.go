...
func CreateLiveChannelSample() {
	...
	simpleCfg := ks3.LiveChannelConfiguration{
		Target: ks3.LiveChannelTarget{
			Type: "HLS",
		},
	}
...
func ListLiveChannelSample() {
	...
	result, err := bucket.ListLiveChannel(ks3.MaxKeys(10))
	if err != nil {
		HandleError(err)
	}
	for _, channel := range result.LiveChannel {
	...
	result, err = bucket.ListLiveChannel(ks3.MaxKeys(10), ks3.Prefix("list-"))
	if err != nil {
		HandleError(err)
	}
	for _, channel := range result.LiveChannel {
