...
func GetObjectSample() {
	...
	body, err = bucket.GetObject(objectKey, ks3.Range(15, 19))
	if err != nil {
		HandleError(err)
	}
	data, err = ioutil.ReadAll(body)
	...
	body, err = bucket.GetObject(objectKey, ks3.IfModifiedSince(pastDate))
	if err != nil {
		HandleError(err)
	}
	body.Close()
	...
	_, err = bucket.GetObject(objectKey, ks3.IfUnmodifiedSince(pastDate))
	if err == nil {
	...
	etag := meta.Get(ks3.HTTPHeaderEtag)
	// Check the content, etag contraint is met, download the file
	body, err = bucket.GetObject(objectKey, ks3.IfMatch(etag))
	if err != nil {
		HandleError(err)
	}
	body.Close()
	...
	_, err = bucket.GetObject(objectKey, ks3.IfNoneMatch(etag))
	if err == nil {
		HandleError(fmt.Errorf("This result is not the expected result"))
	}
	// body.Close()
	...
	err = bucket.DownloadFile(objectKey, "mynewfile-3.jpg", 100*1024, ks3.Routines(3))
	if err != nil {
		HandleError(err)
	}

	// Part size is 100K and 3 coroutines with checkpoint
	err = bucket.DownloadFile(objectKey, "mynewfile-3.jpg", 100*1024, ks3.Routines(3), ks3.Checkpoint(true, ""))
	if err != nil {
		HandleError(err)
	}
	...
	err = bucket.DownloadFile(objectKey, "mynewfile-3.jpg", 100*1024, ks3.Checkpoint(true, "mynewfile.cp"))
	if err != nil {
		HandleError(err)
	}
	...
	err = bucket.GetObjectToFile(objectKey, "myhtml.gzip", ks3.AcceptEncoding("gzip"))
	if err != nil {
		HandleError(err)
	}
